...
__export(main_exports, {
  WalrosPlugin: () => WalrosPlugin,
  default: () => WalrosPlugin
});
...
// src/errors.ts
var WalrosError = class extends Error {
...
var ConfigError = class extends WalrosError {
  constructor(message, details = []) {
    super(message, details);
    this.name = "ConfigError";
  }
};
var CommandExecutionError = class extends WalrosError {
  constructor(message, details = []) {
    super(message, details);
...
// src/types.ts
var CREATE_DIRECTORY_TREE_TYPE = "create_directory_tree";
...
function parseWalrosConfig(yamlText) {
  let raw;
  try {
    raw = load(yamlText);
  } catch (error) {
...
function parseCommand(raw, index, issues) {
  ...
  const type2 = parseNonEmptyString(raw.type, `${path}.type`, issues);
  if (name === null || type2 === null) return null;
  const id = slugifyCommandName(name);
  if (id === "") {
    issues.push(`${path}.name: command name must contain at least one letter or number`);
  }
  if (type2 !== CREATE_DIRECTORY_TREE_TYPE) {
    issues.push(`${path}.type: unsupported command type "${type2}"`);
    return null;
  }
...
function parseRoot(raw, path, issues) {
...
function parseTree(raw, path, issues) {
  if (!isPlainObject(raw)) {
...
function parseOpen(raw, path, issues) {
  if (raw === void 0 || raw === null) return void 0;
  return parseNonEmptyString(raw, path, issues);
}
function parseChildTree(raw, path, issues) {
  const tree = {};
  for (const [name, value] of Object.entries(raw)) {
    const childPath = `${path}.${name}`;
    if (value === null || value === void 0) {
...
function parseNonEmptyString(raw, path, issues) {
...
function yamlParseError(error) {
  if (error instanceof YAMLException) {
    ...
    if ((mark == null ? void 0 : mark.line) !== void 0 && mark.column !== void 0) {
...
// src/directory-tree-executor.ts
...
function validateTreeName(name, configPath) {
  if (name.trim() === "") {
...
// src/directory-tree-executor.ts
var DirectoryTreeExecutor = class {
  constructor(vault) {
    this.vault = vault;
  }
  async createDirectoryTree(rootPath, tree) {
  ...
  buildCreationPlan(rootPath, tree) {
    const plan = [];
    this.collectCreationPlan(rootPath, tree, "tree", plan);
    return plan;
  }
  collectCreationPlan(parentPath, tree, configPath, plan) {
  ...
  validateRootExists(rootPath) {
    const root = rootPath === "" ? this.vault.getRoot() : this.vault.getAbstractFileByPath(rootPath);
  ...
  preflight(plan) {
    ...
    for (const item of plan) {
      ...
      if (existing instanceof import_obsidian2.TFolder || existing instanceof import_obsidian2.TFile) {
...
// src/tree-variables.ts
var VARIABLE_PATTERN = /\{\{\s*([A-Za-z][A-Za-z0-9_]*)\s*\}\}/g;
var PLACEHOLDER_PATTERN = /^\{\{\s*[A-Za-z][A-Za-z0-9_]*\s*\}\}$/;
function extractTreeVariables(tree) {
  const variables = [];
  const seen = /* @__PURE__ */ new Set();
...
function resolveTreeVariables(tree, values) {
  const resolved = {};
...
function extractVariablesFromKey(key) {
...
function resolveTreeKey(key, values) {
  assertValidPlaceholders(key);
  VARIABLE_PATTERN.lastIndex = 0;
  return key.replace(VARIABLE_PATTERN, (_match, variableName) => {
    var _a;
    const value = (_a = values[variableName]) == null ? void 0 : _a.trim();
...
function assertValidPlaceholders(key) {
  let searchIndex = 0;
  while (true) {
...
// src/ui/variable-input-modal.ts
var import_obsidian4 = require("obsidian");
var VariableInputModal = class extends import_obsidian4.Modal {
  constructor(app, variables, onSubmit) {
    super(app);
  ...
  onOpen() {
    ...
    for (const variable of this.variables) {
      new import_obsidian4.Setting(contentEl).setName(variable).addText((text) => {
        if (this.firstInput === void 0) {
          this.firstInput = text;
        }
        text.setPlaceholder(variable).onChange((value) => {
    ...
    new import_obsidian4.Setting(contentEl).addButton((button) => {
      ...
      button.setButtonText("Create").setCta().setDisabled(true).onClick(() => {
        this.submit();
      });
    });
    window.setTimeout(() => {
      var _a;
      (_a = this.firstInput) == null ? void 0 : _a.inputEl.focus();
    }, 0);
  }
  onClose() {
  ...
  submit() {
    ...
    const out = {};
    for (const variable of this.variables) {
      out[variable] = (_b = (_a = this.values.get(variable)) == null ? void 0 : _a.trim()) != null ? _b : "";
    }
    this.close();
    this.onSubmit(out);
    return true;
  ...
  updateCreateButton() {
    var _a;
    (_a = this.createButton) == null ? void 0 : _a.setDisabled(!this.hasAllValues());
  }
};

// src/command-manager.ts
var CONFIG_PATH = ".walros.yaml";
var RELOAD_COMMAND_ID = "reload-walros-config";
var WalrosCommandManager = class {
  constructor(plugin, errorReporter, variablePromptFactory = (app, variables, onSubmit) => new VariableInputModal(app, variables, onSubmit)) {
  ...
  registerStaticCommands() {
    this.plugin.addCommand({
      ...
      name: "Reload config",
      callback: () => {
        this.loadConfiguredCommands("manual").catch((error) => {
          this.errorReporter.reportConfigFailure("manual", error);
        });
      }
    });
  }
  async loadConfiguredCommands(mode) {
  ...
  replaceDynamicCommands(commands) {
    this.unregisterDynamicCommands();
    for (const command of commands) {
      this.registerConfiguredCommand(command);
    }
  }
  registerConfiguredCommand(command) {
    this.plugin.addCommand({
      id: command.id,
      name: command.name,
      callback: () => {
  ...
  promptForRoot(command, folders, variables) {
    if (folders.length === 0) {
      throw new CommandExecutionError("no root folders available");
    }
    const modal = new FolderSuggestModal(this.plugin.app, folders, (folder) => {
      this.promptForVariablesOrExecute(command, folder.path, variables);
    });
    modal.open();
  }
  promptForVariablesOrExecute(command, rootPath, variables) {
    if (variables.length === 0) {
      this.executeCreateDirectoryTree(command, rootPath, {}).catch((error) => {
        this.errorReporter.reportCommandFailure(command.name, error);
      });
      return;
    }
    const modal = this.variablePromptFactory(this.plugin.app, variables, (values) => {
      this.executeCreateDirectoryTree(command, rootPath, values).catch((error) => {
        this.errorReporter.reportCommandFailure(command.name, error);
      });
    });
    modal.open();
  }
  async executeCreateDirectoryTree(command, rootPath, values) {
    ...
    new import_obsidian5.Notice(
      `${command.name}: created ${String(summary.folders)} ${folderNoun} and ${String(summary.files)} ${fileNoun}.`
    );
  }
  getConfiguredRootFolders(rootPaths) {
    return rootPaths.map((rootPath) => this.getRootFolder(rootPath));
  }
  validateOpenVariables(command, treeVariables) {
    if (command.open === void 0) return;
  ...
  resolveOpenPath(command, rootPath, values, resolvedTree) {
    if (command.open === void 0) return void 0;
    ...
    const openPlanItem = this.executor.buildCreationPlan(rootPath, resolvedTree).find((item) => item.path === openPath);
    if (openPlanItem === void 0) {
  ...
  assertNoReservedCommandIds(commands) {
    const conflicts = commands.filter((command) => command.id === RELOAD_COMMAND_ID).map((command) => `command "${command.name}" generates reserved id "${RELOAD_COMMAND_ID}"`);
    if (conflicts.length > 0) {
...
var WalrosErrorReporter = class {
  reportMissingConfig() {
    console.info("Walros: no .walros.yaml found; no dynamic commands registered.");
  }
  reportConfigFailure(mode, error) {
    const message = mode === "startup" ? "Walros config failed to load; dynamic commands unavailable. See console for details." : "Walros config reload failed; keeping existing commands. See console for details.";
    new import_obsidian6.Notice(message);
    this.logError("Walros: config load failed", error);
  }
  reportReloadSuccess(commandCount) {
    const noun = commandCount === 1 ? "command" : "commands";
    new import_obsidian6.Notice(`Walros config reloaded: ${String(commandCount)} ${noun}.`);
  }
  reportCommandFailure(commandName, error) {
    const summary = error instanceof Error ? error.message : String(error);
    new import_obsidian6.Notice(`${commandName} failed: ${summary}`);
...
// src/main.ts
var WalrosPlugin = class extends import_obsidian7.Plugin {
  async onload() {
  ...
  onunload() {
    var _a;
    (_a = this.commandManager) == null ? void 0 : _a.unregisterDynamicCommands();
  }
};
